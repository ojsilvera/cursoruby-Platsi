module Actions
  def self.move_snake(state)
    ...
    # verificar  uqe la siguiente casilla sea valida
  ...
  def self.position_is_valid?(state, position)
    ...
    is_inavalid =  ((position.row >= state.grid.rows || position.row < 0) || (position.col >= state.grid.cols ||
                    position.col < 0))
    return false if is_inavalid
